...
/***/ function(module, exports, __webpack_require__) {
	...
	for(let i =0; i < 50; i+=1){
	    x.push({x: i, y:a*i+b + Math.random()* 50})
	}

	// console.log(result)
	$('<h1>Gradient Descent Polynomial Example</h1>').appendTo('body');


	 
	$( document ).ready(function() {
	    var data = x.map(x => [x.x, x.y])
	    ...
	    let xx = x.map(d => {
	        return {x:d.x, y:d.y, r:15}
	    } )
	    let _linedata = [{x:x1,y:y1} , {x:x2,y:y2}];
